...
if(isset($_GET['id'])  AND !empty($_GET['id'])){
    ...
    $checkIfMenuExist = $bdd->prepare('SELECT * FROM booking WHERE id = ?');
    ...
    if($checkIfMenuExist->rowCount() > 0){
        ...
        $menuInfo = $checkIfMenuExist->fetch();
         //  l'id auteur est le meme que l'id de l'auteur la session qui est connectée ou dde l'admin qui est connectée et bien on pourra exectuer le code
        if($menuInfo['id_auteur'] == isset($_SESSION['id']) || $_SESSION['mdp']){

            $titremenu = $menuInfo['date'];
            $descriptionmenu = $menuInfo['heure'];
